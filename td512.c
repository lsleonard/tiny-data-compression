...
int32_t td512(unsigned char *inVals, unsigned char *outVals, const uint32_t nValues, const uint32_t betterCompression)
{
    ...
    if (nValues <= 64)
    {
        ...
        outVals[0] = (unsigned char)(nValues << 1); // first bit indicates <= 64 vals, followed by number values
        if (nValues < 6)
        {
            // output 1 to 5 values without compression
            memcpy(inVals, outVals+1, nValues);
    ...
    if (nBytesRemaining > 0)
    {
        ...
        // pass/fail bit is 0
        memcpy(inVals+inputOffset, outVals+outputOffset, nBytesRemaining);
    ...
    if (nValues <= 320)
    {
        ...
        outVals[1] |= passFail << 2; // next 5 bits record pass/fail status for up to 5 compressed blocks
    }
    else
    {
        // nValues 321 to 512
        ...
        // attempt to compress pass/fail bits to fit in 2 bytes
        ...
        else
        {
            ...
            memmove(outVals+3, outVals+2, nValues);
...
int32_t td512d(unsigned char *inVals, unsigned char *outVals, uint32_t *totalBytesProcessed)
{
    ...
    else
    {
        ...
        else
        {
            ...
            passFail = inVals[2];
            inputOffset = 3; // point past first three byters
        }
    }
